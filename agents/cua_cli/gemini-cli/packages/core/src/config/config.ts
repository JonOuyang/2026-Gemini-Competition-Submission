...
// Simplified types (removed code_assist dependency)
...
export interface Experiments {
  ...
  flags: Record<string, ExperimentFlag>;
}
...
export const ExperimentFlags = {
  ...
  ENABLE_NUMERICAL_ROUTING: 'enable_numerical_routing',
  CLASSIFIER_THRESHOLD: 'classifier_threshold',
  BANNER_TEXT_NO_CAPACITY_ISSUES: 'banner_text_no_capacity_issues',
...
export class Config {
  ...
  async refreshAuth(authMethod: AuthType) {
    ...
    // Simplified: removed code_assist server integration
    // Just set experiments to empty and allow preview for API key users
    this.experimentsPromise = Promise.resolve();
  ...
  async refreshUserQuota(): Promise<RetrieveUserQuotaResponse | undefined> {
    // Simplified: removed code_assist server integration
    // API key users always have access to preview models
    this.setHasAccessToPreviewModel(true);
    return undefined;
  }
