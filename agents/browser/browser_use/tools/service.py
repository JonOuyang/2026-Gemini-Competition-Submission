	...
	def __init__(
		...
		async def input(
			...
			async def upload_file(
				...
				# Normalize common local path forms (~, relative basename) before whitelist checks.
				raw_path = params.path
				expanded_path = os.path.abspath(os.path.expandvars(os.path.expanduser(raw_path)))
				if expanded_path in available_file_paths:
					params = UploadFileAction(index=params.index, path=expanded_path)
				elif raw_path not in available_file_paths and not os.path.isabs(raw_path):
					# If model emitted only basename, map to a whitelisted absolute path.
					for allowed_path in available_file_paths:
						if os.path.basename(str(allowed_path)) == raw_path:
							params = UploadFileAction(index=params.index, path=str(allowed_path))
							break
